...
/**
 * *@description Este script maneja CORS y la carga de archivos.
 * Permite que orígenes específicos realicen solicitudes y verifica la autorización y los tipos de archivo para las cargas.
 * */
...
header(
	"Access-Control-Allow-Headers: Content-Type, Content-Length, Accept-Encoding, X-CSRF-Token, authorization, accept, origin, apiclient, Cache-Control, X-Requested-With",
);
...
switch ($requestMethod) {
	case "POST":
		...
		$debug["apiClient"] = $apiClient;

		$expectedToken = apache_getenv("AUTH_TOKEN");
		$expectedApiClient = apache_getenv("API_CLIENT");

		$debug["expectedToken"] = $expectedToken;
		$debug["expectedApiClient"] = $expectedApiClient;

		$response["debug"] = $debug;

		if (!verifyAuthorization($authToken, $apiClient)) {
			http_response_code(401);
			$response["response"] = "asdas";
			$response["debug"] = $debug;
			echo json_encode($response);
			return;
		}
